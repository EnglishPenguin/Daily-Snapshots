...
def run():
    ...
    df = pd.DataFrame(df, columns=['MRN', 'VisitIDCode', 'VisitType', 'RetrievalStatus', 'RetrievalDescription'])  
    df['Business Status'] = np.where(
    (df['RetrievalDescription'] == 'C00 - Completed') | 
    (df['RetrievalDescription'] == 'C00 - C00-Completed'),
    'Success',
    'Exception'
    )
    ...
    df2 = pd.DataFrame(df,columns=['MRN', 'VisitIDCode', 'VisitType', 'StatusCode_Description', 'Business Status'])
    ...
    df2.rename(columns={'MRN': 'Account Number', 'VisitIDCode': 'Plan Code', 'VisitType': 'Plan Name', 'StatusCode_Description': 'Exception Description'}, inplace=True)
    ...
    plt.ylabel('Count')
    plt.title('Counts by Retrieval Description')
    plt.xticks(rotation=20, ha='right', fontsize=6)
    # Add data labels to the bars
    for i, v in enumerate(status_code_counts):
        ax.text(i, v, str(v), ha='center', va='bottom')
    # Specify the file path and name
    bar_graph_filename = f'U:/zORCCA TEAM/Daily Analysis/eCare Next/{fd_MMDDYYYY}_MCR_PartB_Inactive.png'
    ...
    <p>Link to File can be found: <a href="file:///{file_path}/eCareNext_MCRAdvantage_Outbound_{fd_MMDDYYYY}.xlsx">here</a></p>
    ...
    mail.Subject = f'Medicare Advantage Daily Snapshot - {fd_MM_DD_YYYY}'
